...
// const axios = require('axios');   //for aws http calls for file downloads
...
//       .where({ status: 0 })
//       .whereNull('locked_at')  //  record is not already locked
//       .orderBy('id', 'asc')     
//       .forUpdate()                //locks rows so that other process cant acess parallely
//       .first();
...
const XLSX = require('xlsx');
const Joi = require('joi');
const AWS = require('aws-sdk'); // Optional, used when getting objects
const fs = require('fs');
const path = require('path');
const { promisify } = require('util');
require('dotenv').config();
...
// Joi schema for validating records
const recordSchema = Joi.object({
  product_name: Joi.string().required(),
  category_name: Joi.string()
    .valid(
...
const CHUNK_SIZE = 100; //  chunk size for processing valid records
...
// Create an Excel file for invalid records
const createInvalidRecordsExcel = async (invalidRecords, fileName) => {
  try {
    const formattedRecords = invalidRecords.map(record => ({
      ...record,
...
const getRecords = async () => {
  ...
  try {
    ...
    // Handle valid records in chunks
    if (validRecords.length > 0) {
      for (let i = 0; i < validRecords.length; i += CHUNK_SIZE) {
        const chunk = validRecords.slice(i, i + CHUNK_SIZE);
        try {
          await Promise.all(chunk.map(row => addProduct(row)));
        } catch (err) {
          console.error(`Error inserting chunk:`, err);
          invalidRecords.push(...chunk);
        }
      }
    }
