...
export class NavbarComponent implements OnInit {
  ...
  ngOnInit() {
    this.navbarService.getUserById().subscribe({
      next: (data) => {
        ...
        // console.log(`hii ${this.profilePicUrl}`);
        console.log('url is ', this.profilePicUrl);
      },
      error: (err) => {
                ...
                this.http
                  ...
                  .subscribe(
                    (getResponse: any) => {
                      ...
                      this.navbarService
                        .updateUserProfilePic(this.user.user_id, {
                          profile_pic: getUrl,
                        })
                        .subscribe(
                          (updateResponse) => {
                            console.log(
                              'User profile picture URL updated in database:',
                              updateResponse
                            );
                            alert(
                          ...
                          (error) => {
                            console.error(
                              'Error updating user profile picture in database:',
                              error
                            );
                            alert(
  ...
  logout() {
    ...
    // sessionStorage.removeItem('r_authToken');
