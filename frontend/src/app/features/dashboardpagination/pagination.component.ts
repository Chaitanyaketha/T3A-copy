...
export class PaginationComponent implements OnInit {
  ...
  async upload(file: any) {
    try {
      const response: any = await this.fileUploadService
        .getPresignedUrlForUpload(file.name, file.type)
        .toPromise();
      await this.fileUploadService
        .uploadFileToS3(response.url, file, file.type)
        .toPromise();
      const getUrlResponse: any = await this.inventoryService
        .getPresignedUrl_forget(file.name, file.type)
        .toPromise();
      const url = getUrlResponse.urls[0].url;
      console.log(url);
      await this.http
        .post<any>('http://localhost:4000/api/v1/user/imports/add', {
          url: url,
          file_name: file.name,
        })
        .toPromise();
  ...
  adddata() {
    this.data.forEach((data: any) => {
      this.inventoryService.addProduct(data).subscribe({
        next: (response) => {
          this.loadInventory();
  ...
  loadInventory() {
    ...
    this.inventoryService.getInventory(queryParams).subscribe({
      next: (response) => {
  ...
  saveEdit(item: any): void {
    ...
    this.http
      .put(
        `http://localhost:4000/api/v1/user/editProduct/${item.product_id}`,
        updatedProduct
      )
      .subscribe(
        (response) => {
  ...
  deleteItem(item: any): void {
    this.http
      .delete(
        `http://localhost:4000/api/v1/user/deleteProduct/${item.product_id}`
      )
      .subscribe(
        (response) => {
          this.loadInventory();
  ...
  decreaseQuantity(item: any) {
    ...
    if (item.quantity_in_stock > 0) {
      ...
      console.log(payload);
      this.inventoryService.decreaseQuantity(payload).subscribe({
        next: (response: any) => {
          this.loadInventory();
  ...
  increaseQuantity(item: any) {
    ...
    console.log(payload);
    this.http
      .post('http://localhost:4000/api/v1/user/cart/increaseQuantity', payload)
      .subscribe({
        next: (response: any) => {
          this.loadInventory();
  ...
  removeFromCart(item: any) {
    ...
    this.http
      .delete(
        `http://localhost:4000/api/v1/user/cart/remove/${Cart_ID}?product_id=${product_id}`
      )
      .subscribe({
        next: (response) => {
    ...
    this.http
      ...
      .subscribe({
        next: (response) => {
          ...
          this.fileUploadService.uploadFileToS3(url, file, fileType).subscribe(
            () => {
  ...
  uploadToS3(url: string, file: File): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.http
        .put(url, formData, { headers: { 'Content-Type': file.type } })
        .subscribe({
          next: () => {
            console.log('File uploaded successfully');
            this.fileUploadService
              .getPresignedUrlsForDownload([file.name])
              .subscribe((data) => {
  ...
  onSubmit() {
    ...
    if (this.addProductForm.valid && this.productImageUrl) {
      ...
      this.http
        .post('http://localhost:4000/api/v1/user/addProduct', productData)
        .subscribe({
          next: () => {
  ...
  decQuantity(item: any) {
    if (item.quantity_in_stock <= 0) alert('not possible');
    else item.quantity_in_stock--;
  }
  save(selectedItems: any) {
    this.http
      .post(
        'http://localhost:4000/api/v1/user/cart/addItems',
        this.selectedItems
      )
      .subscribe({
        next: (response) => {
        ...
        next: () => {
          ...
          this.http
            ...
            .subscribe({
              ...
              error: (err) => {
                console.error(
                  'Error getting access URL for edited image:',
                  err
                );
              },
            });
